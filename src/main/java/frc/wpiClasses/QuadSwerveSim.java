...
public class QuadSwerveSim {
     ...
     * @param robotMOI         Effective moment of inertia of the robot, in
     *                         kilogram-meters-squared
     * @param modules          Set of swerve modules controlling the drivetrain.
     *                         Order must be 0=FL, 1=FR, 2=BL, 3=BR
     */
    public QuadSwerveSim(double wheelBaseWidthM, double wheelBaseLengthM, double robotMasskg,
                         double robotMOI, List<SwerveModuleSim> modules) {
        this.m_modules = modules;
        m_robotToModuleTL = Arrays.asList(new Translation2d(wheelBaseWidthM / 2,
                                                            wheelBaseLengthM / 2), new Translation2d(wheelBaseWidthM / 2,
                                                                                                     -wheelBaseLengthM / 2), new Translation2d(-wheelBaseWidthM / 2,
                                                                                                                                               wheelBaseLengthM / 2), new Translation2d(-wheelBaseWidthM / 2,
                                                                                                                                                                                        -wheelBaseLengthM / 2));
        m_robotToModule = Arrays.asList(new Transform2d(m_robotToModuleTL.get(FL),
                                                        new Rotation2d(0.0)), new Transform2d(m_robotToModuleTL.get(FR),
                                                                                              new Rotation2d(0.0)), new Transform2d(m_robotToModuleTL.get(BL),
                                                                                                                                    new Rotation2d(0.0)), new Transform2d(m_robotToModuleTL.get(BR),
                                                                                                                                                                          new Rotation2d(0.0)));
