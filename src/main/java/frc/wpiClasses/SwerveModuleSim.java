...
public class SwerveModuleSim {
     ...
     * 
     * @param azimuthMotor         Motor driving the azimuth angle portion of the
     *                             mechanism
     * @param wheelMotor           Motor driving the wheel to rotate
     * @param wheelRadiusM         Wheel radius, in meters
     * @param azimuthGearRatio     Motor rotations per one azimuth module rotation.
     *                             Should be greater than zero
     * @param wheelGearRatio       Motor rotations per one wheel rotation. Should be
     *                             greater than zero
     * @param azimuthEncGearRatio  Encoder rotations per one azimuth module
     *                             rotation. Should be 1.0 if you have a good swerve
     *                             module.
     * @param wheelEncGearRatio    Encoder rotations per one wheel rotation.
     * @param treadStaticCoefFric  Static coefficient of friction of the wheel tread
     *                             and ground interface
     * @param treadKineticCoefFric Kinetic coefficient of friction of the wheel
     *                             tread and ground interface
     * @param moduleNormalForce    Downward force the robot and its mechanisms exert
     *                             on this module.
     * @param azimuthEffectiveMOI  Effective moment of inertia of the azimuth
     *                             module. The Azimuth motion is modeled as a simple
     *                             flywheel.
     */
    public SwerveModuleSim(DCMotor azimuthMotor, DCMotor wheelMotor, double wheelRadiusM,
                           double azimuthGearRatio, double wheelGearRatio,
                           double azimuthEncGearRatio, double wheelEncGearRatio,
                           double treadStaticCoefFric, double treadKineticCoefFric,
                           double moduleNormalForce, double azimuthEffectiveMOI) {
        this.m_azmthMotor = new SimpleMotorWithMassModel(azimuthMotor, azimuthGearRatio,
                                                         azimuthEffectiveMOI);
        this.m_wheelMotor = new MotorGearboxWheelSim(wheelMotor, wheelGearRatio, wheelRadiusM,
    ...
    /**
     * @return Returns the current physical rotation count of the azimuth encoder
     *         axle shaft in total rotations since last reset - not wrapped. IE, if
     *         you spin your module exactly two and a half times in the positive
     *         direction, this will return 2.5 (not 0.5).
     */
    ...
    void reset(Pose2d initModulePose) {
        ...
        m_curAzmthAngle = Rotation2d.fromDegrees(0); // TODO check if this needs to pull from the
                                                     // Pose2d for angle offset?
    }
    ...
    /**
     * Get a vector of the velocity of the module's contact patch moving across the
     * field.
     */
    Vector2d getModRelTransVel(double dtSeconds) {
        double xvel = (m_curModulePose.getTranslation().getX() - m_prevModulePose.getTranslation().getX()) / dtSeconds;
        double yvel = (m_curModulePose.getTranslation().getY() - m_prevModulePose.getTranslation().getY()) / dtSeconds;
    ...
    /**
     * Gets the modules on-axis (along wheel direction) force, which comes from the
